  ...
  savePluginNodes(0, () => {
    saveNodes (() => {
       saveMainInfo(() => {
          ...
          if (controlPluginsWindow) controlPluginsWindow.destroy();
...
readCyConfigProp((style) => {
  ...
  cy = cytoscape({
    ...
    style: cytoscape.stylesheet()
        .selector('node')
        .css({
          //'content' : (Config.nodeName) ? 'data(name)' : null,
...
function unresizePluginElements() {
  let pluginList = Avatar.Plugin.getList();
	for (let i=0; i < pluginList.length; i++) {
		if (pluginList[i]._script.unresize) {
      pluginList[i]._script.unresize(list => {
        unresizePluginList = _.union( unresizePluginList, list);
      });
    }
	}
}
...
function addlog(id, source, msg, timeout, type) {
    ...
    client_options.timeout = (!timeout) ? 10000 : timeout;
    try {
  ...
  if (mappedClients && mappedClients.length > 0) {
      cy.nodes().forEach(function( ele ) {
        if (ele.hasClass('mapped')) {
            ...
            if (even) {
                for (i in even) {
                    info("Mapped client", even[i].split(',')[0], " is gone");
...
function beforeNodeMenu(callback) {
  let pluginList = Avatar.Plugin.getList();
	for (let i=0; i < pluginList.length; i++) {
  ...
  /*  var handler = function (e) {
    ...
    if (!flagTooltip) {
      try {
        ...
        console.log('Impossible d\'afficher le menu circulaire', err);
      }
    }
...
function resize(e) {
  ...
  if (cy) {
    cy.nodes().forEach(function( ele ){
      let elemClass = ele.classes();
      if ((elemClass && elemClass.length > 0 && !_.contains(unresizePluginList, elemClass[0])) && !_.contains(unresizePluginList, "ALL")) {
...
function clientAction (action) {
  ...
  switch (action) {
    ...
    case 'restart':
      ...
      break;
    case 'quit':
      Avatar.Socket.getClientSocket(selectedNode.data('name')).emit('quit');
      break;
    case 'shutdown':
      Avatar.Socket.getClientSocket(selectedNode.data('name')).emit('shutdown');
      break;
    case 'lockRoom':
    ...
    case 'speaker':
      ...
      speakerWindow.loadFile('./assets/html/soundLevel.html');
...
function nodeSettings(noMappedTab) {
  ...
  readProp('./resources/core/Avatar.prop', (AvatarConfig) => {
      ...
      nodeSettingsWindow.loadFile('./assets/html/nodeSettings.html');
...
function restart_Avatar() {
  readProp('./resources/core/Avatar.prop', (AvatarConfig) => {
      if (AvatarConfig.restart > 0) {
        ...
        info('Redémarrage automatique toutes les', Math.round(24/AvatarConfig.restart), "heures et " + AvatarConfig.mn_restart + " minutes" );
      	if (job) job.stop();
      	job = new cron(delay, function(done) {
            reload_all(true, false);
      	},null, true);
      } else
...
function reload_all(serveur, clients) {
  if (clients) {
    var tblclients = Avatar.Socket.getClients();
    _.map(tblclients, element => {
      element.Obj.emit('restart');
    });
  }
  if (serveur) {
    setTimeout(function() {
...
function close_all(serveur, clients) {
  if (clients) {
    var tblclients = Avatar.Socket.getClients();
    _.map(tblclients, element => {
      element.Obj.emit('quit');
    });
  }
  if (serveur) {
    setTimeout(function() {
...
function shutdown_all(serveur, clients) {
  if (clients) {
    var tblclients = Avatar.Socket.getClients();
    _.map(tblclients, element => {
  ...
  if (serveur) {
    setTimeout(function() {
      let cmd = path.resolve (__dirname+"/lib/nircmd/nircmd");
    	exec(cmd+' exitwin poweroff', (err, stdout, stderr) => {
        if (err)
          error("Je n'ai pas pu arrêter le pc, message:", stderr);
      });
      job = null;
    }, 3000);
  }
...
function settings() {
  ...
  readProp('./resources/core/Avatar.prop', (AvatarConfig) => {
      ...
      settingsWindow.loadFile('./assets/html/settings.html');
      ...
      .on('ConfigSave', (event, arg) => {
          ...
          if (arg.waitAction.time != AvatarConfig.waitAction.time || arg.default.client != AvatarConfig.default.client || arg.http.ip != AvatarConfig.http.ip || arg.http.port != AvatarConfig.http.port || arg.udp.port != AvatarConfig.udp.port || arg.interface != AvatarConfig.interface || arg.interfaceSpeak_timer != AvatarConfig.interfaceSpeak_timer || arg.restart != AvatarConfig.restart || (AvatarConfig.nodeName == null && arg.nodeName == true) || AvatarConfig.nodeName != arg.nodeName || (AvatarConfig.sensor == null && arg.sensor == true) || AvatarConfig.sensor != arg.sensor) {
            newConfigMessage = "";
            if (arg.waitAction.time != AvatarConfig.waitAction.time)
                newConfigMessage += "Le paramètre de synchronisation des actions a changé.<br>";
            
            if (arg.interfaceSpeak_timer != AvatarConfig.interfaceSpeak_timer || (AvatarConfig.nodeName == null && arg.nodeName == true) || AvatarConfig.nodeName != arg.nodeName || (AvatarConfig.sensor == null && arg.sensor == true) || AvatarConfig.sensor != arg.sensor)
...
function installedPlugins() {
  ...
  pluginsWindow.loadFile('./assets/html/installedPlugins.html');
...
let controlPluginsWindow;
function controlPlugins() {
  if (controlPluginsWindow) {
    controlPluginsWindow.show();
    return;
  }

  var reloadChangePlugins = false;
  ...
  var style = {
    ...
    height: 550,
    icon: 'resources/app/images/Avatar.png',
    title: 'Ordonner les Plugins'
  }

  controlPluginsWindow = new BrowserWindow(style);
  controlPluginsWindow.loadFile('./assets/html/controlPlugins.html');
  controlPluginsWindow.setMenu(null);

  //controlPluginsWindow.openDevTools();

  controlPluginsWindow.once('ready-to-show', () => {
      controlPluginsWindow.show();
  })

  controlPluginsWindow.on('closed', function () {
    ipcMain.removeAllListeners('reoderWindowPlugins');
    ipcMain.removeAllListeners('quitWindowPlugins');
    ipcMain.removeAllListeners('reloadChangePlugins');
    if (reloadChangePlugins == true) {
      let msg = "L'ordre et/ou l'activation des plugins a changé<br>Voulez-vous redémarrer le serveur Avatar maintenant (préconisé) ?";
      SaveConfig (null, msg, false);
    }
    controlPluginsWindow = null;
    reloadChangePlugins = false;
  })

  ipcMain.on('quitWindowPlugins', (event, arg) => {
    controlPluginsWindow.close();
  });

  ipcMain.on('reoderWindowPlugins', (event, arg) => {
    event.returnValue = controlPluginsWindow.id;
  });

  ipcMain.on('reloadChangePlugins', (event, arg) => {
    reloadChangePlugins = true;
    event.returnValue = true;
  });
...
function librairyPlugins() {
  ...
  librairyWindow.loadFile('./assets/html/librairyPlugins.html');
